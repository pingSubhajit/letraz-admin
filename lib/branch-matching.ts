...
export const matchBranchWithLinearIssues = async (
  ...
  try {
    ...
    if (!linearClient) {
      console.warn('No Linear credentials available (missing cookie or env)')
      return {
        branchName,
        linearIssueId,
        matchedIssue: null,
    ...
    if ((issues as any).nodes?.length === 0) {
      console.log('Linear issues lookup returned no results', {
        teamKey,
        issueNumber,
        filteredByRepoTeamId: !!repositoryData.linearTeamId,
      })
      return {
        branchName,
        linearIssueId,
    ...
    console.log('Raw Linear issue object:', {
      id: issue.id,
      ...
      projectMilestone: (issue as any).projectMilestone,
      milestone: (issue as any).milestone
    })
    ...
    console.log('Processing Linear issue data for transformation...')
    ...
    console.log('Resolved objects:', {
      stateObj: stateObj ? { name: (stateObj as any).name } : null,
      assigneeObj: assigneeObj ? { id: (assigneeObj as any).id, name: (assigneeObj as any).displayName } : null,
      labelsObj: labelsConn,
      teamObj: teamObj ? { id: (teamObj as any).id, name: (teamObj as any).name } : null,
      projectObj: projectObj ? { id: (projectObj as any).id, name: (projectObj as any).name } : null
    })
    ...
    console.log('Transformed issue data:', {
      id: matchedIssue.id,
      identifier: matchedIssue.identifier,
      title: matchedIssue.title,
      state: matchedIssue.state,
      assignee: matchedIssue.assignee,
      labelsCount: matchedIssue.labels.length,
      labels: matchedIssue.labels,
      project: matchedIssue.project,
      hasMilestone: !!matchedIssue.milestone
    })

    // Extract milestone and GitHub issue data from Linear issue
    console.log('Processing Linear issue attachments...')
    console.log('Issue data:', {
      id: issue.id,
      identifier: issue.identifier,
      title: issue.title,
      hasProject: !!projectObj,
      projectData: projectObj ? { id: (projectObj as any).id, name: (projectObj as any).name } : null
    })
    ...
    if (issueProjectMilestone) {
      try {
        ...
        if (m) {
          ...
          console.log('Found milestone on issue:', matchedIssue.milestone)
        }
      } catch (e) {
        console.warn('Failed to resolve projectMilestone on issue:', e)
      }
    }
    ...
    if (projectObj) {
      try {
        ...
        console.log('Checking project for milestone...')
        console.log('Project data keys:', Object.keys(projectData))
        console.log('Project milestone field:', projectData.milestone)
        console.log('Project milestone via projectMilestone:', projectData.projectMilestone)
        ...
        if (milestoneObj) {
          ...
          console.log('Found milestone:', matchedIssue.milestone)
        } else {
          console.log('No milestone found on project')
        }
      } catch (e) {
        console.warn('Failed to resolve project milestone:', e)
      }
    }

    // Check for GitHub issue attachments and other integration data
    console.log('Looking for GitHub issue attachments and other data...')
    console.log('Issue integration data:', {
      integrationSourceType: issue.integrationSourceType,
      hasExternalId: !!(issue as any).externalId,
      hasBranchName: !!(issue as any).branchName,
      branchName: (issue as any).branchName
    })
    ...
    if (issue.integrationSourceType && (issue.integrationSourceType as string).includes('github')) {
      console.log('GitHub integration detected on Linear issue')
    ...
    try {
      ...
      console.warn('Failed to inspect attachments for GitHub links:', e)
    }
    ...
    console.error('Error matching branch with Linear issues:', error)
    return {
      branchName,
      linearIssueId,
      matchedIssue: null,
...
export const getOrCreateGitHubLinearMapping = async (
  ...
  try {
    ...
    console.error('Error creating GitHub-Linear mapping:', error)
    return false
  }
}
